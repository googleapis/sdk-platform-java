...
# install OS tools
RUN apt-get update && \
    apt-get install -y openjdk-17-jdk maven jq libxml2-utils && \
    apt-get clean

COPY library_generation /src
ADD https://repo1.maven.org/maven2/com/google/googlejavaformat/google-java-format/${JAVA_FORMAT_VERSION}/google-java-format-${JAVA_FORMAT_VERSION}-all-deps.jar /src/owlbot/downloaded/google-java-format.jar
...
RUN chmod -R a+rw /src/owlbot/downloaded
