...
public class SampleNames {
    ...
    public static String createSampleName(String methodName, String identifier){
        return String.format("%s%s",
                JavaStyle.toUpperCamelCase(methodName),
                JavaStyle.toUpperCamelCase(identifier));
    }

    public static String createSampleName(String methodName, List<MethodArgument> arguments){
        String args = arguments.stream()
                .map(arg -> JavaStyle.toUpperCamelCase(arg.type().reference().simpleName())).collect(Collectors.joining());
