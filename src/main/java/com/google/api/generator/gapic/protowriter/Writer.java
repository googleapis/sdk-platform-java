...
public class Writer {
  ...
  public static CodeGeneratorResponse write(
    ...
    // TODO: Once directory structure for configured snippets is defined, doublecheck this works
    writeConfiguredSnippets(context, gapicPackageInfo, jos);
...
// This is for the prototype only, will need to be updated to mimic current autogenerated snippet directory
// TODO: @alicejli update this once it's been decided where these snippets should live relative to the non-configured autogenerated snippets
  private static void writeConfiguredSnippets(GapicContext context, GapicPackageInfo gapicPackageInfo, JarOutputStream jos){
    ...
    for (GapicSnippetConfig snippetConfig : context.snippetConfigs()) {
      JarEntry jarEntry =
              new JarEntry(
                      String.format(
                              "%s/configuredSnippets/%s.java",
                              packagePath,
                              GapicSnippetConfig.getConfiguredSnippetSnippetName(snippetConfig).replaceAll("\\s","")));
      String executableSampleCode = SampleCodeWriter.writeConfiguredSnippet(snippetConfig);
      try {
        jos.putNextEntry(jarEntry);
        jos.write(executableSampleCode.getBytes(StandardCharsets.UTF_8));
      } catch (IOException e) {
        throw new GapicWriterException(
                String.format(
                        "Could not write sample code for %s.: %s",
                        GapicSnippetConfig.getConfiguredSnippetSnippetName(snippetConfig), e.getMessage()),
                e);
      }
    }
  }
