...
out_layer_folder=${library_gen_out}/${destination_path}
mkdir -p "$library_gen_out"
mkdir -p "$out_layer_folder"
...
######################################################
#cd "${GOOGLEAPIS_ROOT}"
#"${protoc_path}"/protoc "--java_out=${library_gen_out}/${proto_path}/java_proto.jar" ${proto_files}
