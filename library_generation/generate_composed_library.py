...
def generate_composed_library(
    ...
    if library.googleapis_commitish is not None:
        ...
        print(
            "using library-specific googleapis commitish: "
            f"{googleapis_commitish}"
        )
    else:
        print(
            "using common googleapis_commitish: "
            f"{config.googleapis_commitish}"
        )
    ...
    if not (os.path.exists(f"{output_folder}/google") and
            os.path.exists(f"{output_folder}/grafeas")):
    ...
    for gapic in library.gapic_configs:
        ...
        build_file_folder = Path(f"{output_folder}/{gapic.proto_path}")\
            .resolve()
        print(f"build_file_folder: {build_file_folder}")
        client_inputs = parse_build_file(build_file_folder, gapic.proto_path)
        effective_arguments += [
            "--proto_only",
            client_inputs.proto_only,
            "--gapic_additional_protos",
            client_inputs.additional_protos,
            "--transport",
            client_inputs.transport,
            "--rest_numeric_enums",
            client_inputs.rest_numeric_enum,
            "--gapic_yaml",
            client_inputs.gapic_yaml,
            "--service_config",
            client_inputs.service_config,
            "--service_yaml",
            client_inputs.service_yaml,
            "--include_samples",
            client_inputs.include_samples,
        ]
        ...
        print(f"Generating library from {gapic.proto_path} "
